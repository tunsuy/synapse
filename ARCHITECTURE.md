...
│   ├── store/              # Store 扩展点
│   │   ├── store.go        # Store 接口定义
│   │   └── local/          # 参考实现：Local Store
│   ├── indexer/            # Indexer 扩展点
│   │   ├── indexer.go      # Indexer 接口定义
│   │   └── bm25/           # 参考实现：BM25 Indexer
│   ├── consumer/           # Consumer 扩展点
│   │   ├── consumer.go     # Consumer 接口定义
│   │   └── hugo/           # 参考实现：Hugo Consumer
│   └── auditor/            # Auditor 扩展点
│       ├── auditor.go      # Auditor 接口定义
│       └── default/        # 参考实现：Default Auditor
├── pkg/                    # 共享工具包
│   ├── model/              # 领域模型（RawContent, KnowledgeFile 等）
│   ├── link/               # 双向链接解析工具
│   └── jsonrpc/            # JSON-RPC 通信工具
├── api/                    # MCP Server 定义
│   └── mcp/                # MCP 工具定义与处理器
├── configs/                # 配置模板
│   ├── schema.yaml         # 默认 Schema 模板
│   └── config.yaml         # 默认 Config 模板
├── docs/                   # 项目文档
│   ├── roadmap.md          # 路线图
│   └── decision.md         # 方案决策记录
└── test/                   # 测试工具与集成测试
...
| 作用域 | 路径 | 用途 |
|--------|------|------|
| 全局 | `~/.synapse/config.yaml` | 用户级默认配置 |
| 项目 | `.synapse/config.yaml` | 知识库级配置（优先级更高） |
