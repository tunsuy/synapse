...
- **工作流规则**：知识处理的自动化规则
- **质量标准**：知识库健康检查的基准
- **展示元数据**：每种页面类型的 `Emoji`（目录图标）和 `Example`（示例文件名），用于 README 动态生成等场景

所有扩展点共同遵守 Schema 定义的规范，修改 Schema 即修改所有 AI 助手的行为。

#### PageTypeDefinition 关键字段

| 字段 | 类型 | 必填 | 说明 |
|------|------|------|------|
| `name` | string | ✅ | 类型名称（如 `topic`、`entity`） |
| `directory` | string | ✅ | 对应的知识库目录（如 `topics/`） |
| `description` | string | ✅ | 类型描述 |
| `emoji` | string | ❌ | 目录展示用的 emoji 图标，用于 README 生成 |
| `example` | string | ❌ | 示例文件名，用于 README 表格展示 |
...
│   ├── claude/             # Claude Code Skill（SYNAPSE.md）
│   ├── common/             # 通用 Skill 模板（共享知识规范）
